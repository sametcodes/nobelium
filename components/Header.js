...
const NavBar = () => {
  ...
  const links = [
    { id: 0, name: locale.NAV.MAIN, to: "/", show: true },
    { id: 1, name: locale.NAV.INDEX, to: "/blog", show: true },
    { id: 2, name: locale.NAV.PROJECTS, to: "/projects", show: true },
    { id: 3, name: locale.NAV.ABOUT, to: "/about", show: false },
        ...
        {links.map(
          (link) =>
            link.show && (
              <li
                key={link.id}
                className={`block ml-4 text-black dark:text-gray-50 ${
                  pathname === link.to ? "border-b border-b-slate-900" : ""
                }`}
              >
