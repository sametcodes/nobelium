...
import { defaultMapImageUrl, NotionRenderer as Renderer } from "react-notion-x";
...
const mapImageUrl = (url, block, link) => {
  if (url.startsWith(link) && block.type === "image") {
    return url;
  }
  ...
  return (
    <>
      ...
      <Renderer
        ...
        mapImageUrl={(url, block) => mapImageUrl(url, block, config.link)}
